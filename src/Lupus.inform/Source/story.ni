...
[Oberklasse für go, besitzt flag für das Schließen nach 1 Runde]
...
[Raum Hangar]
Hangar is a room. hangar is inside the hauptebene.
...
Docking Bay is inside the hauptebene.
...
Gamma Junction is inside the hauptebene.
The description of Gamma Junction is "Korridor im äußeren Ring. Es ist ein Bodenfenster zu sehen.
Nordwestlich befindet sich Gamma Delta Corridor, südwestlich Gamma Beta Corridor und darunter der Hangar. Eine Treppe führt hoch zum Xeno Lab, die durch eine Luke versperrt ist. Die Storage Area befindet sich westlich von Gamma Junction.".
...
[Tür - Gamma Junction]
Southwest of Gamma Junction is a Sicherheitsbarriere called door_gambeta2gamma. 
The description of door_gambeta2gamma is "Eine Sicherheitsbarriere des äußeren Ringes. Zum Öffnen und Schließen muss das Türpanel benutzt werden. Sonst könnte dir das MobiTab behilflich sein..". 
The door_gambeta2gamma can be LOCKED or UNLOCKED.
The door_gambeta2gamma is LOCKED.
The door_gambeta2gamma is CLOSED.

[Raum Gamma Delta Corridor]
Gamma Delta Corridor is a room. 
Gamma Delta Corridor is inside the hauptebene.
...
Delta Junction is inside the hauptebene.
...
door_delta2solar is closed and locked.
...
Gamma Beta Corridor is inside the hauptebene.
Southwest of door_gambeta2gamma is Gamma Beta Corridor.
...
Northeast of Gamma Beta Corridor is door_gambeta2gamma.  

[Raum Alpha Delta Corridor]
Alpha Delta Corridor is a room.
Alpha Delta Corridor is inside the hauptebene.
...
Southwest of Alpha Delta Corridor is door_aldelta2alpha.

Northeast of door_aldelta2alpha is Alpha Delta Corridor. 


[Raum Alpha Junction]
Alpha Junction is a room.
Alpha Junction is inside the hauptebene.
The description of Alpha Junction is "Korridor im äußeren Ring. Es ist ein Bodenfenster im Raum. Eine Treppe führt hoch zum Med-Lab, allerdings ist sie durch eine Luke versperrt. Nordöstlich befindet sich Alpha Delta Corridor, südöstlich Alpha Beta Corridor und darunter der Duty Room.".
...
Northeast of Alpha Junction is a Sicherheitsbarriere called door_aldelta2alpha. 
The description of door_aldelta2alpha is "Eine Sicherheitsbarriere des äußeren Ringes. Zum Öffnen und Schließen muss das Türpanel benutzt werden. Sonst könnte dir das MobiTab behilflich sein..".
door_alpha2med is locked and closed.

The door_aldelta2alpha can be LOCKED or UNLOCKED.
The door_aldelta2alpha is LOCKED.
The door_aldelta2alpha is CLOSED.
...
Alpha Beta Corridor is inside the hauptebene.
...
Beta Junction is inside the hauptebene.
...
Delta Greenhouse is inside the linke seitenebene.
...
Storage Room is inside the linke seitenebene.
...
[Storage Room - Alle Kontaminierte versammeln Algorithmus]
Every Turn when scene3_aktiv is true:
	[laufe durch alle kontaminierten und prüfe ob einer nicht im Storage Room sind -> falls doch dann wird die flag false gesetzt]
	let versammelt be true;
	repeat with mensch running through the Mensches:
		if mensch is not in the Storage Room and mensch is KONTAMINIERT:
			now versammelt is false;
		if mensch is following: [Kontaminierte dürfen nicht folgen]
			now versammelt is false;
	[alle Kontaminierten waren im Storage Room]
	if versammelt is true:
		[prüfe ob Barry den Raum verlassen hat]
		if barry is not in the Storage Room:
			[Spiel geschafft!]
			say "[line break][line break][line break]Herzlichen Glückwunsch![line break][line break] Du hast das Spiel erfolgreich beendet. Nach einigen weiteren Herausforderungen sind Percy und Barry von der Lupus Station geflohen.[line break]";
...
[Alpha AI]
Alpha AI is a room.
Alpha AI is inside the linke seitenebene.
...
Transporter Raum is inside the linke seitenebene.
...
Beta Greenhouse is inside the linke seitenebene.
...
Storage Area is inside the linke seitenebene.
...
[Delta AI]
Delta AI is a room.
Delta AI is inside the linke seitenebene.
...
Main Generator is inside the linke seitenebene.
...
[Raum Med Lab]
Med Lab is a room.
Med Lab is inside the hauptebene.
...
Engineering Lab is inside the hauptebene.
Up of the door_beta2engin is Engineering Lab. 
door_beta2engin is locked and closed.
...
[Raum Solar Lab]
Solar Lab is a room.
Solar Lab is inside the hauptebene.
...
[Raum Dienstraum]
Duty Room is a room.
Duty Room is inside the hauptebene.
...
Crew Quarter is inside the hauptebene.
...
Fitness Raum is inside the linke seitenebene.
...
[Raum Cafeteria]
Cafeteria is a room.
Cafeteria is inside the linke seitenebene.
...
Briefing Room is inside the rechte seitenebene.
...
[Com Base]
Com Base is a room.
Com Base is inside the hauptebene.
...
Second Generator is inside the linke seitenebene.
...
Antenna Array is inside the rechte seitenebene.
...
[Umkleiderkammer]
Umkleidekammer is a room. The description of Umkleidekammer is "Die Umkleidekammer eines ranghohen Offiziers.".
door_hang2umkleide is a closed, locked door. door_hang2umkleide is west of Hangar and east of Umkleidekammer.
...
Bridge is a room.
Bridge is inside the rechte seitenebene.
...
[Raum Xeno Lab]
Xeno Lab is a room.
Xeno Lab is inside the hauptebene.
Up of Gamma Junction is a door called Xeno Luke. Up of Xeno Luke is Xeno Lab. 
...
Xeno Luke is a closed door with printed name "Xeno Luke". 
Xeno Luke is below Xeno Lab. 
[hat Attribut BLOCKED oder UNBLOCKED (je nach Palette]
Xeno Luke is either BLOCKED or UNBLOCKED. Xeno Luke is UNBLOCKED. 
[Beschreibung ist je nach Attribut anders]
The description of Xeno Luke is "[if Xeno Luke is BLOCKED] Die Luke des Xeno-Labs. Die Palette blockiert den Schließvorgang, sodass man nun hinein kann. [otherwise if Xeno Luke is UNBLOCKED] Die Luke des Xeno-Labs. Sie kann ohne passenden Transponder nicht geöffnet werden. Nach dem Öffnen schließt sie sich sofort wieder, sodass man nicht hinein kann ohne die Luke zu blockieren.".
[TÜR XENO LAB]
[Xeno Luke kann nur geöffnet werden wenn der Spieler den Kittel trägt]
Instead of opening Xeno Luke when the player is not wearing the Laborkittel:
say "Du benötigst den passenden Transponder.[line break]";
[Xeno Luke kann nur passiert werden, wenn sie blockiert ist]
Instead of going through the Xeno Luke when the Xeno Luke is UNBLOCKED:
...
Every Turn when the Xeno Luke is open:
	if counter >= 1 and Xeno Luke is open and Xeno Luke is UNBLOCKED:
		say "Die Xeno Luke ist von selbst wieder zugegangen.[line break]";
		now Xeno Luke is closed;
		now counter is 0;
		stop;
...
Every Turn when scene1_aktiv is true:
	...
	if the player is in the Gamma Junction AND Xeno Luke is open:
		now Pfeifen is AKTIV;
	if Alarm is DEAKTIV:
		now Pfeifen is DEAKTIV;
...
Instead of pushing Blinkender Knopf:
	if Blinkender Knopf is BLINKT:
		...
		say "Durch den Knopfdruck sind der Alarm und das Pfeifen nun weg. Es hat sich eine Klappe in der Wand geöffnet und eine Phiole war dahinter verborgen.[line break]";
...
[Phiole]
Phiole is a thing in Klappe in der Wand. It is portable.The description of Phiole is "Eine Phiole mit rosafarbenen Nebel darin".
Instead of taking the Phiole:
	...
	remove Phiole from play;
	now Percy is KONTAMINIERT;
	now Blinkender Knopf is BLINKT NICHT;


[
	AP A 15 - Mobitab, Ausweis, Panel
	Autor: Ibrahim Karaki
]
[MobiTab]
Mobitab is a device in Spind. The Description of Mobitab is "Ein Mobitab. Eine Art Tablet mit vielen nützlichen Funktionen. Es kann mit einem Tür verbunden werden, um es zu beschädigen.". Mobitab is portable.
...
[Mobitab Kontaminierten Alarm - Prüfe bei jedem Raumwechsel ob sich ein Kontaminierter im Raum befindet und weise Spieler darauf hin]
[Raumwechsel Flag (wird in der Abfrage (Before going) aktiviert)]
mobi_room_change is a truth state that varies. mobi_room_change is false.
Every Turn when mobi_room_change is true:
	if the player is carrying mobitab:
		repeat with konta running through the Mensches in the location of the player:
			if konta is KONTAMINIERT:
				say "Alarm: Es befinden sich ein oder mehrere Kontaminierte in diesem Raum!";
				now mobi_room_change is false;
				stop;
	now mobi_room_change is false;
...
[Panel Init]
Panel door_gamma2gamma is a Türpanel and a part of door_gamma2gamma with printed name "Panel door_gamma2gamma".
Panel door_gambeta2gamma is a Türpanel and a part of door_gambeta2gamma with printed name "Panel door_gambeta2gamma".
Panel door_gamma2beta is a Türpanel and a part of door_gamma2beta with printed name "Panel door_gamma2beta".
...
Panel door_aldelta2alpha is a Türpanel and a part of door_aldelta2alpha with printed name "Panel door_aldelta2alpha".
...
[Kittel der Leiche wegnehmen]
Instead of taking the laborkittel:
	remove laborkittel from play;
	now the player is carrying laborkittel;
	say "Du hast der Leiche den Laborkittel abgenommen.";
...
Dekontaminationskabine is a room. 
door_dekonta2med is a open door. door_dekonta2med is west of med lab and east of Dekontaminationskabine .
...
Instead of closing door_dekonta2med:
	...
	if kontam_counter is greater than 1:
		...
		stop;
	[nur Percy im Raum]
	now door_dekonta2med is closed;
...
[Prüfe ob Xeno Lab]
	if room is Xeno Lab:
		now Xeno Luke is BLOCKED;
		now Xeno Luke is open;
...
To react:
	[flag für ausgabe, damit nicht mehrfach ausgegeben wird]
	let output be FALSE;
	repeat with mensch running through the Mensches in the location of the player:
		if mensch is KONTAMINIERT:
			...
			if mensch is UNATTENTIVE:
				...
				stop;
	if output is TRUE:
		say "Die Kontaminierten scheinen dich bemerkt zu haben.";
...
[Raumwechsel Algo
-> Mobitab Flag für Raumwechsel wird gesetzt
]
[vor Raumwechsel]
Before going [somewhere]:
	now turn_counter is 0;
	now mode is 0;
	now mobi_room_change is TRUE;
[nach Raumwechsel]
[ -> Counter und Modus der Kontaminierten zurücksetzen]
after going:
...
[Klasse]
Kontaminierter is a kind of Mensch. The description of Kontaminierter is "Ein Kontaminierter. Pass auf, dass er dich nicht kontaminiert.".
Kontaminierter is KONTAMINIERT.
[Kontaminierte im Spiel]
Kontaminierter A is a Kontaminierter in the Fitness Raum.
Kontaminierter B is a Kontaminierter in the Fitness Raum.
Kontaminierter C is a Kontaminierter in the Fitness Raum.
Kontaminierter D is a Kontaminierter in the Alpha Delta Corridor.
Kontaminierter E is a Kontaminierter in the Alpha Beta Corridor.
Kontaminierter Kapitän is a Kontaminierter in the Bridge.
Kontaminierter Offizier is a Kontaminierter in the Bridge.
Kontaminierter Wissenschaftler is a Kontaminierter in the Med Lab.
...
Percy is a Mensch in the Hangar. The description of Percy is "Der Pilot und Spezialist für Vakuumeinsätze Percy Braden.".
Barry is a Mensch. Barry is in the Fähre. The description of Barry is "Der Ingenieur Barry McIntyre.".